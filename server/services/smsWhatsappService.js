 ...
 * Channels:
 *   1. In-app notifications (stored in DB, pushed via Socket.IO)
 *   2. Email via SendGrid HTTP API (avoids Render's SMTP port blocks)
 */
...
/**
 * Send email via SendGrid HTTP API — works on Render (no SMTP port issues).
 */
async function sendEmailHttp({ email, subject, text, html }) {
	...
	const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
		...
		body: JSON.stringify({
			...
			from: { email: fromAddr, name: "AgriConnect" },
			subject,
			content: [
	...
	if (!response.ok) {
		...
		throw new Error(`SendGrid API ${response.status}: ${err}`);
	}
...
async function sendEmail({ userId, email, type, body, emailBody, emailSubject }) {
	...
	const text = emailBody
		? emailBody.replace(/<[^>]*>/g, "").trim()  // strip HTML tags for plain text
		: body;
	...
	try {
		await sendEmailHttp({ email, subject, text, html });
...
export async function notifyUser({ userId, type, smsBody, whatsappBody, emailBody, emailSubject }) {
	...
	// 2. Send email via SendGrid HTTP API
	if (user.email) {
		await sendEmail({
			userId,
			email: user.email,

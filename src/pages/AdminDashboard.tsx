...
function AdminDashboard() {
  ...
  const handleCreateAgroShop = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(msg || 'Failed to register shop');
    }
  };
  ...
  const handleCreateAgroInput = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = axios.isAxiosError(e) ? e.response?.data?.error : undefined;
      toast.error(msg || 'Failed to create input');
    }
  };
  ...
  const handleReviewExport = async (id: string, status: 'APPROVED' | 'REJECTED', reason?: string) => {
    try {
      ...
      const msg = axios.isAxiosError(e) ? e.response?.data?.error : undefined;
      toast.error(msg || 'Failed to update application');
    }
  };
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {dashboardData.recentTransactions.map((tx) => (
                          <tr key={tx.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">UGX {tx.amount.toLocaleString()}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
